...
namespace Generator.LazyStatic;

[Generator]
public sealed class LazyStaticGenerator : IIncrementalGenerator
{
    ...
    private static bool IsValidClassSyntax(SyntaxNode node) =>
        ...
        && cls.AttributeLists.Any(attrList =>
            attrList.Attributes.Any(attr => attr.Name.ToString() == "LazyStatic")
        );
    ...
    )
    {
        foreach (var classSyntax in classes)
        {
            ...
            var usingDirectives = classSyntax
                .SyntaxTree.GetRoot()
                .DescendantNodes()
    ...
    )
    {
        ...
        if (propertyName.Length == 0 || fieldName == "ServiceCache")
            return; // 排除 ServiceCache 字段
        ...
        // 显式类型转换（替代直接强制转换）
        if (grandParent is not FieldDeclarationSyntax fieldDeclaration)
            throw new InvalidOperationException(
                "Variable's grandparent is not a FieldDeclarationSyntax."
            );
        ...
        sourceBuilder.AppendLine(
            $@"    public static {fieldType} {propertyName} => Resolve<{fieldType}>();"
        );
    }
    ...
    {
        var current = classSyntax.Parent;
        while (current != null)
        {
            switch (current)
            {
